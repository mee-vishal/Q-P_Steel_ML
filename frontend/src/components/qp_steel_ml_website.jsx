...
function PredictionTool() {
  ...
  const handlePredict = async () => {
    ...
    // Heuristic k-NN simulation based on paper insights
    let predicted = 4;
    const { C, Si, Mn, TMAE, QT, PT } = inputs;
    if (C < 0.15) predicted = 2;
    else if (Si < 0.5) predicted = 0;
    else if (Mn > 4.0) predicted = 2;
    else if (QT > 300) predicted = 0;
    else if (PT > 450 && PT < 600) predicted = 5;
    else if (Si >= 0.75 && Si <= 2.25 && Mn <= 4.0) predicted = 4;
    else if (TMAE < 0.01) predicted = 0;
    else predicted = 4;

    const cls = microstructureClasses[predicted];
    const confidence = predicted === 4 ? Math.round(47 + Math.random() * 15) : Math.round(75 + Math.random() * 20);

    setResult({ cls, confidence, inputs: { ...inputs } });

    // Generate Grok-style suggestion
    let suggestion = "";
    if (predicted === 4) {
      suggestion = "⚠️ The model predicts {M, RA} microstructure — this is the most uncertain class (f1=0.51). Confidence is relatively low. Consider verifying experimentally. For a cleaner {M, RA} prediction, try: Si ≈ 1.8 wt%, C ≈ 0.68 wt%, Mn ≈ 1.72 wt%, with one-step Q&P at QT = PT ≈ 200°C (below Ms). These parameters align with the validated HC9 steel composition in the research.";
    } else if (predicted === 0) {
      suggestion = "✅ The model confidently predicts {M, B, RA}. This often occurs at lower Si or higher QT. If bainite is undesirable, consider increasing Si above 0.75 wt% and reducing QT below 250°C to shift toward a cleaner {M, RA} microstructure. The high Si suppresses carbide formation and bainite growth.";
    } else if (predicted === 2) {
      suggestion = "ℹ️ Low carbon content is driving {M, F, RA} prediction. Ferrite formation is expected in low-C, high-Mn steels. If you want to suppress ferrite, increase carbon to > 0.4 wt% or reduce Mn below 4 wt%.";
    } else if (predicted === 5) {
      suggestion = "ℹ️ High partitioning temperature promotes carbide formation, yielding {M, RA, C}. To obtain a carbide-free microstructure, keep PT below 200°C. The paper indicates PT < 200°C favors carbide-free {M, RA}.";
    } else {
      suggestion = `✅ The model predicts ${cls.label} with reasonable confidence. This class has an f1-score of ${classMetrics[predicted].f1.toFixed(2)} on the test set — reliable for engineering decisions. Review the class confusion map for similar neighboring predictions.`;
    }
    setGrokSuggestion(suggestion);
    setLoading(false);
  };
